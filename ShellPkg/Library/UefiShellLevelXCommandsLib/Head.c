/** @file
  Main file for Head shell level X function.
  ...
  )
{
  ...
  BOOLEAN             AsciiMode;
  BOOLEAN             UnicodeMode;
  CONST CHAR16        *LineCountStr;
  CONST CHAR16        *LineCountStrPtr;
  UINTN				  LineCount;
  ...
  if (EFI_ERROR(Status)) {
    if (Status == EFI_VOLUME_CORRUPTED && ProblemParam != NULL) {
      ShellPrintHiiEx(-1, -1, NULL, STRING_TOKEN (STR_GEN_PROBLEM), gShellLevelXHiiHandle, L"head", ProblemParam);
    ...
    LineCountStr = ShellCommandLineGetValue(Package, L"-n");
	
	//
	// Parse the parameter of line count number.
	// Default behaviour: Print 10 lines.
	//
	LineCountStrPtr = LineCountStr;
	if (LineCountStr != NULL) {
		LineCount = ShellStrToUintn (LineCountStrPtr);
	}
	else{
		LineCount = 10;
	}
    ...
    if (AsciiMode && UnicodeMode) {
      ShellPrintHiiEx(-1, -1, NULL, STRING_TOKEN (STR_GEN_PARAM_INV), gShellLevelXHiiHandle, L"head", L"-a & -u");
      ShellStatus = SHELL_INVALID_PARAMETER;
   } else if (ShellCommandLineGetRawValue(Package, 1) == NULL) {
      //
      // we insufficient parameters
      //
      ShellPrintHiiEx(-1, -1, NULL, STRING_TOKEN (STR_GEN_TOO_FEW), gShellLevelXHiiHandle, L"head");
      ShellStatus = SHELL_INVALID_PARAMETER;
    } else {
      //
      ...
      for ( ParamCount = 1, Param = ShellCommandLineGetRawValue(Package, ParamCount)
        ...
        if (EFI_ERROR(Status)) {
          ShellPrintHiiEx(-1, -1, NULL, STRING_TOKEN (STR_GEN_FILE_OPEN_FAIL), gShellLevelXHiiHandle, L"head", (CHAR16*)Param);
        ...
        if (ShellStatus == SHELL_SUCCESS) {
          ...
          if (FileList == NULL || IsListEmpty(&FileList->Link)) {
            ShellPrintHiiEx(-1, -1, NULL, STRING_TOKEN (STR_GEN_FILE_NF), gShellLevelXHiiHandle, L"head", Param);
            continue;
          } else {
            //
            ...
            for ( Node = (EFI_SHELL_FILE_INFO*)GetFirstNode(&FileList->Link)
              ...
              if (EFI_ERROR(Node->Status)){
                ShellPrintHiiEx(-1, -1, NULL, STRING_TOKEN (STR_GEN_FILE_OPEN_FAIL), gShellLevelXHiiHandle, L"head", Node->FileName);
                ShellStatus = SHELL_NOT_FOUND;
                continue;
              }
              ...
              if (FileHandleIsDirectory(Node->Handle) == EFI_SUCCESS) {
                ShellPrintHiiEx(-1, -1, NULL, STRING_TOKEN (STR_GEN_IS_DIR), gShellLevelXHiiHandle, L"head", Node->FileName);
                ShellStatus = SHELL_NOT_FOUND;
                continue;
              }
              ...
              // do it
              //
              Status = HeadFileByHandle (Node->Handle, AsciiMode, UnicodeMode, LineCount);
              if (EFI_ERROR(Status)) {
                ShellPrintHiiEx(-1, -1, NULL, STRING_TOKEN (STR_HEAD_ERROR), gShellLevelXHiiHandle, L"head", Node->FileName);
                ShellStatus = SHELL_INVALID_PARAMETER;
              }
